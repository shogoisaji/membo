...
class ColorUtils {
  ...
  static Color moreLight(Color color) {
    const double lightValue = 0.1;
    double alpha = HSVColor.fromColor(color).alpha;
    double hue = HSVColor.fromColor(color).hue; // 色相を正しく取得
    double saturation =
        (HSVColor.fromColor(color).saturation - lightValue).clamp(0.0, 1.0);
    double value = (HSVColor.fromColor(color).value + lightValue)
        .clamp(0.0, 1.0); // 明度を正しく減少させる
    Color newColor = HSVColor.fromAHSV(alpha, hue, saturation, value).toColor();
    return newColor;
  }
